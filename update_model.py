...
def main():
    ...
    for t in all_teams:
        ...
        opponents = [row['away_team'] if row['home_team'] == t else row['home_team'] for _, row in team_schedule.iterrows()]
    ...
    for t in all_teams:
        ...
        wins, losses = team_records[t]['wins'], team_records[t]['losses']
        ...
        ranking_rows.append({
            ...
            'SOS': sos_str, 'Rating': round(rating, 1), 'BasePlayoff': max(1.0, min(99.0, playoff_prob))
        })
    ...
    # 5. Machine Learning Training & Out-Of-Sample Evaluation
    ...
    X = all_completed_games[features].fillna(0)
    y = all_completed_games['home_win']

    # Evaluate Model Accuracy (Out of Sample)
    if len(X) > 10:
        ...
        metrics_df = pd.DataFrame([{'accuracy': acc, 'brier_score': brier, 'log_loss': ll}])
        metrics_df.to_csv("model_metrics.csv", index=False)
    else:
        # Failsafe mock data if not enough games
        pd.DataFrame([{'accuracy': 0.50, 'brier_score': 0.25, 'log_loss': 0.69}]).to_csv("model_metrics.csv", index=False)

    # Train final model on ALL data for max predictive power
